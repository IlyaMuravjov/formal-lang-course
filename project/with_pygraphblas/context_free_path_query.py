...
from project.with_pygraphblas.bool_decomposed_nfa import BoolDecomposedNFA
from project.with_pygraphblas.bool_decomposition import BoolDecomposition
...
def _cfpq_with_matrix(
    ...
    while True:
        ...
        for (head, var1, var2) in variable_productions:
            added = is_reachable[var1] @ is_reachable[var2]
            if not changed:
                added = added.eadd(
                    added, mask=is_reachable[head], desc=pygraphblas.descriptor.C
                )
                changed = changed or added.reduce_bool()
            is_reachable[head] += added
        if not changed:
            break
    return {
...
def cfpq_with_tensor(
    ...
    while True:
        ...
        added_transitions: Dict[
            pyformlang.finite_automaton.Symbol, Tuple[List[int], List[int]]
        ] = defaultdict(lambda: ([], []))
        for (start, finish) in transitive_closure:
            ...
            starts, finishes = added_transitions[
                pyformlang.finite_automaton.Symbol(start_var.value)
            ]
            starts.append(bool_decomposed_graph.state_to_idx(start_graph_state))
            finishes.append(bool_decomposed_graph.state_to_idx(finish_graph_state))
        for (symbol, (starts, finishes)) in added_transitions.items():
            bool_decomposed_graph.adj_bool_decomposition[
                symbol
            ] += pygraphblas.Matrix.from_lists(
                I=starts,
                J=finishes,
                nrows=bool_decomposed_graph.state_count,
                ncols=bool_decomposed_graph.state_count,
            )
    return {
